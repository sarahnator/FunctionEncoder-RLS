...
class MultiDimQuadraticDataset(BaseDataset):
    ...
    def sample(self) -> Tuple[  torch.tensor, 
        ...
        with torch.no_grad():
            ...
            for dim in range(self.output_dim):
                ...
                all_example_ys.append(example_ys)
                all_query_ys.append(query_ys)
            
            all_example_ys = torch.cat(all_example_ys, dim=-1)
            all_query_ys = torch.cat(all_query_ys, dim=-1) # (n_func, n_sample, 2)

            return example_xs, all_example_ys, query_xs, all_query_ys, {"As":As, "Bs" : Bs, "Cs": Cs}
