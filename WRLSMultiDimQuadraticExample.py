...
# load_path = 'logs/WRLS_multi_dim_example/least_squares/shared_model/2025-03-21_14-35-09'
residuals = args.residuals
if load_path is None:
    logdir = f"logs/WRLS_multi_dim_example/{train_method}/{'shared_model' if not args.parallel else 'parallel_models'}/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
else:
    logdir = load_path
...
if load_path is None:
    # create the model
    model = WRLSFunctionEncoder(input_size=dataset.input_size,
...
with torch.no_grad():
    ...
    # plot one dimension
    y_hat_initial = y_hat_initial[:, plot_output_dim_num].unsqueeze(-1)
    ...
    def update(frame):
        ...
        # plot one dimension
        y_hat = y_hat[:, plot_output_dim_num].unsqueeze(-1)
